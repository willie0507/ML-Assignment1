...
for i, test_image in enumerate(test_data):  # Calculate SAD, SSD
    ...
    test_data[i] = (test_data[i][0], (train_data[SAD.index(min(SAD))][2], train_data[SSD.index(min(SSD))][2]))
    if test_data[i][1][0] == int(test_data[i][0][5:7]):
        SAD_result += 1
    print("SAD Acc: ", SAD_result / (i+1))
    if test_data[i][1][1] == int(test_data[i][0][5:7]):
        SSD_result += 1
    print("SSD Acc: ", SSD_result / (i+1))
